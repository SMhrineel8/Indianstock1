...
async def search_stock(q: str = Query(..., min_length=1)):
    ...
    for stock in NIFTY50:
        clean_stock = stock.upper().replace(".NS", "").replace(".BO", "")
        if clean_stock.startswith(cleaned_query):
            results.insert(0, stock)
        elif cleaned_query in clean_stock:
            results.append(stock)

    results = list(dict.fromkeys(results))
    return {"query": q, "results": results[:15], "count": len(results)}
